...
const media = [
  ...
  {
    summary: "The Ukrainian government raised more than $600,000 through nonfungible token sales that will be used to rebuild museums, theaters and other cultural institutions destroyed since Russia’s invasion in February.",
    outlet: "Bloomberg",
    url: "https://www.bloomberg.com/news/articles/2022-04-01/ukrainian-government-raises-600-000-through-museum-nft-sales",
  },
  {
    summary: "With a brewing partnership with the Ministry of Culture, the funds raised from the auction exclusive for NFT holders shall be used to recover the damaged historical and cultural heritage of Ukraine.",
    outlet: "CoinQuora",
  ...
  {
    summary: "But not only are the NFTs intended to raise funds, 100% of which, according to the website, will go directly to the official crypto-accounts of the Ministry of Digital Transformation of Ukraine to support army and civilians, but will also bring to light Russia’s invasion of Ukraine.",
    outlet: "Fortune",
    ...
    <div className="mobile:my-60px tablet:my-100px desktop:my-120px">
      <div className="flex flex-row items-end justify-between">
        ...
        : <div className="flex flex-row mb-15">
            <button 
              ...
              className={`${disabledLeft ? 'opacity-20' : 'opacity-100'} tablet:w-48px tablet:h-48px mobile:w-40px mobile:h-40px flex items-center justify-center border-2 border-carbon rounded-full mr-25px`}
              >
            ...
            <button 
              ...
              className={`${disabledRight ? 'opacity-20' : 'opacity-100'} tablet:w-48px tablet:h-48px mobile:w-40px mobile:h-40px flex items-center justify-center border-2 border-carbon rounded-full`}
            >
      ...
      ? <div className="flex flex-row mt-25px">
          <button 
            disabled={disabledLeft}
            onClick={() => setStartIdx(startIdx - itemsOnPage)}
            className={`${disabledLeft ? 'opacity-20' : 'opacity-100'} mobile:w-40px mobile:h-40px flex items-center justify-center border-2 border-carbon rounded-full mr-40px`}
            >
            <img src="left_arrow.svg" alt="left_arrow" />
          </button>
          <button 
            onClick={() => setStartIdx(startIdx + itemsOnPage)}
            disabled={disabledRight}
            className={`${disabledRight ? 'opacity-20' : 'opacity-100'} mobile:w-40px mobile:h-40px flex items-center justify-center border-2 border-carbon rounded-full`}
          >
            <img src="right_arrow.svg" alt="right_arrow" />
          </button>
        </div>
