...
const Ambassadors = () => {
  ...
  const blurbText = isDesktop ? 'Ambassadors' : 'Ambassad\nors'
  ...
  return (
    <div className="font-rlight desktop:py-120px tablet:py-[96px] mobile:py-60px">
      ...
      <div className="flex desktop:flex-row tablet:flex-row mobile:flex-col justify-between flex-wrap">
        {data.map(datum => (
          <div className="desktop:basis-1/4 tablet:basis-1/2 mobile:basis-1">
            <div className="flex flex-col desktop:mt-40px tablet:mt-40px mobile:mt-32px" key={datum.name}>
            ...
            <div className="font-rblack inline-flex flex-row cursor-pointer text-14px desktop:leading-48px tablet:leading-48px mobile:leading-40px border-b-4 hover:border-b-4 hover:border-solid border-transparent hover:border-carbon" onClick={() => {
        ...
        <div className="mobile:hidden tablet:flex desktop:flex desktop:basis-1/4 tablet:basis-1/2">
          <img src="img/dots-ambassadors.svg" className="w-[248px] h-96px mt-auto" alt="dots" />
        </div>
      </div>
    </div>
  );
