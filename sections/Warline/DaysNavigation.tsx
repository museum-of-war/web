...
function DaysNavigation(props: DaysNavigationProps) {
  const { dayData, onNextDayClickHandler, onPrevDayClickHandler, daysCount, direction = "vertical" } = props;
  const wrapperCn = direction === 'horizontal'
    ? "flex-row"
    : "flex-col mb-10px"
  return (
    <div className={`flex justify-between items-center font-rblack mt-24px ${wrapperCn}`}>
      {dayData.dayNo > 1 && (
        <span
      ...
      {dayData.dayNo < daysCount && (
        <span
          ...
          className={`flex flex-row items-center hover:cursor-pointer ${direction === "horizontal" ? "ml-32px" : ""}`}
        >
          <LinkButton>Day {dayData.dayNo + 1}</LinkButton>
